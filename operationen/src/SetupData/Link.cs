...
namespace Operationen.Setup
{
	...
	public class Link
	{
		...
		public static bool Exists(string DirectoryPath, string LinkPathName)
		{
            ...
            string NewFileName = SpecialDir.FullName + System.IO.Path.DirectorySeparatorChar + originalfile.Name + ".lnk";
		...
		public static void Update(Environment.SpecialFolder folder, string TargetPathName, string TargetWorkingDir, string LinkPathName, bool install)
		{
			// Get some file and directory information
            Link.Update(Environment.GetFolderPath(folder), TargetPathName, TargetWorkingDir, LinkPathName, install);
		}
		...
		public static void Update(string DirectoryPath, string TargetPathName, string TargetWorkingPath, string LinkPathName, bool Create)
		{
			// Get some file and directory information
			DirectoryInfo SpecialDir = new DirectoryInfo(DirectoryPath);
			// First get the filename for the original file and create a new file
			...
			if (Create) // If the link doesn't exist, create it
			{
				...
				{ 
                    ...
                    link.WorkingDirectory = TargetWorkingPath;
