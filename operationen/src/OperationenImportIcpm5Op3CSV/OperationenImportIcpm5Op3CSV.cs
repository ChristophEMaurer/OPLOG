	...
	public partial class OperationenImportPlugin : OperationenImport
	{
        ...
        public override string OPImportDescription()
        {
            string s = string.Format("Dieses Import-Plugin importiert Operationen aus einer Text-Datei mit Codierung '{0}'."
                ...
                + "\r\n         <OPSKode4>;<OPSBezeichnung4>;<OPSKode5>;<OPSBezeichnung6>;"
            ...
            )
        {
            if (strNachname.Length > 0)
            {
                ...
                else
                {
                    ...
                    _oEvent.State = EVENT_STATE.STATE_INFO;
                    _oEvent.StateText = string.Format("Unerwarteter Wert für Funktion {0} in Spalte {1}, Eintrag wird ignoriert in Datenzeile: {2}",
        ...
        private void ImportLine(string strLine)
        {
            ...
            //
            // 8          9         10          11       12         13
            // <OPSKode3>;<OPSBez3>;<OPSKode4>;<OPSBez4>;<OPSKode5>;<OPSBez5>;
            //
            // 14          15           16
            // <Operateur1;<Operateur2>;<Assistent>
            ...
            if (arLine.Length == 17)
            {
                ...
                string strNameOP1 = arLine[14];
                string strNameOP2 = arLine[15];
                string strNameAss = arLine[16];
                ...
                if (BeginnUndEnde2DateTime(strDatum, strZeitBeginn, strZeitEnde, "Datum/Zeit", strLine, ref dtBeginn, ref dtEnde))
                {
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        {
                            // Operateur1
                            HandleBeteiligter(strLine, strFallzahl, 14, strNachnameOP1, strVornameOP1, 14, OP_FUNCTION.OP_FUNCTION_OP, dtBeginn, dtEnde, strOpsKode, strOpsText);
                            if (_oEvent.Abort)
                            {
                                break;
                            }
                            ...
                            // Operateur2
                            HandleBeteiligter(strLine, strFallzahl, 15, strNachnameOP2, strVornameOP2, 15, OP_FUNCTION.OP_FUNCTION_OP, dtBeginn, dtEnde, strOpsKode, strOpsText);
                            if (_oEvent.Abort)
                            {
                                break;
                            }
                            ...
                            // Ass
                            HandleBeteiligter(strLine, strFallzahl, 16, strNachnameAss, strVornameAss, 16, OP_FUNCTION.OP_FUNCTION_ASS, dtBeginn, dtEnde, strOpsKode, strOpsText);
                            if (_oEvent.Abort)
                            {
                                break;
                            }
            ...
            else
            {
                // Not 17 columns
                _oEvent.State = EVENT_STATE.STATE_INFO;
                _oEvent.StateText = "Keine 17 Spalten in Datenzeile, Datenzeile wird ignoriert: " + strLine;
                FireImportOPEvent(_oEvent);
            }
        }
    }
